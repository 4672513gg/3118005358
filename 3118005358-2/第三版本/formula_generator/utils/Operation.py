...
def inspect(answer_file, expression_file):
    ...
    try:
        ...
        for item_b in answer_content:
            ...
            # 每行答案的序号
            answer_sqe = int(item_b.split('. ')[0])

            answer =item_b.split('.')[1]
            # 找到对应的习题的行数
        ...
        # 保存结果
        save_inspect(correct_seq, wrong_seq,expression_file[-5])
...
def save_exercise(expressions_list, order):
    ...
    try:
        ...
        print('Exercise{}.txt 创建失败，请重新检查'.format(order))
...
def save_answer(answers_list, order):
    ...
    try:
        ...
        print('Answer{}.txt 创建失败，请重新检查'.format(order))


# 保存比较结果 传入正确列表以及错误列表
def save_inspect(correct_list, wrong_list,NO):

    inspect_file = './docs/Grade{}.txt'.format(NO)
    try:
        ...
        print("作业更改完成，结果存放在 'Grade{}.txt' 中".format(NO))
    except IOError:
        print('Grade{}.txt 创建失败，请重新检查'.format(NO))

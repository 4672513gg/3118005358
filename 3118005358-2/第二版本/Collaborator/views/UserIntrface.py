...
class InitWindows(object):
    ...
    def get_(self):
        ...
        try:
            ...
            if r < 2 or n < 1:
                ...
                tk.messagebox.showinfo("Info", "生成时间较长,请稍等")
                Generator(n, r, self.order).multi_processor()
            else:
                Generator(n, r, self.order).multi_processor()
            tk.messagebox.showinfo("Info", "成功")
            self.open_explorer()
        except ValueError:
            tk.messagebox.showinfo("Info", "输入非法")
    ...
    def init_widgets(self):
        ...
        lb_input_range = tk.Label(self.root, text="请输入参与运算最大值", anchor="center", width=self.btn_width)
        ...
        # All buttons
        btn_commit_generate = tk.Button(self.root, text="生成题目", command=lambda: self.thread_event(self.get_), anchor="center", width=self.btn_width)
        btn_commit_inspect = tk.Button(self.root, text="检查答案", command=lambda: self.thread_event(self.inspect_dual_file), anchor="center", width=self.btn_width)
    ...
    def select_answer_file(self):
        ...
        flag_ord = self.answer_file_name[-5]
    ...
    def inspect_dual_file(self):
        try:
            if self.expression_file_name != '' and self.answer_file_name != '':
                ...
                tk.messagebox.showinfo("Info", "失败！")
                return False
        except:
            tk.messagebox.showinfo("Info", "文本文件不合法")

    def get_inspect_info(self):
        filename = './docs/Grade{}.txt'.format(flag_ord)
        ...
        with open(filename, 'rb') as f:
            ...
            if tk.messagebox.askyesno("Info", "检查成功！\n"
                ...
                os.system("open /Users/rebecca/Collaborator/docs/Grade{}.txt".format(flag_ord))
